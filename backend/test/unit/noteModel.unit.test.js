...
describe('noteModel (unit, mocked MySQL pool)', () => {
  ...
  it('create() inserts a note and returns it with the new id', async () => {
    ...
    expect(params).to.deep.equal([1, 'Title', 'Body', null]);
    expect(note).to.deep.equal({ id: 42, userId: 1, title: 'Title', content: 'Body', category: null });
  });

  it('create() stores the category when one is given', async () => {
    poolStub.query.resolves([{ insertId: 43 }]);

    const note = await noteModel.create({ userId: 1, title: 'Title', content: 'Body', category: 'study' });

    const [, params] = poolStub.query.firstCall.args;
    expect(params).to.deep.equal([1, 'Title', 'Body', 'study']);
  ...
  it('findAllByUser() adds a category filter when one is passed', async () => {
    poolStub.query.resolves([[{ id: 1, user_id: 1, category: 'study' }]]);

    await noteModel.findAllByUser(1, 'study');

    const [sql, params] = poolStub.query.firstCall.args;
    expect(sql).to.match(/WHERE user_id = \? AND category = \?/);
    expect(params).to.deep.equal([1, 'study']);
  });

  it('countByCategory() groups the user\'s notes by category', async () => {
    poolStub.query.resolves([[{ category: 'study', count: 3 }, { category: null, count: 1 }]]);
  ...
  it('clearCategory() nulls out the category on matching notes', async () => {
    ...
    const [sql, params] = poolStub.query.firstCall.args;
    expect(sql).to.match(/UPDATE notes SET category = NULL WHERE user_id = \? AND category = \?/);
    expect(params).to.deep.equal([1, 'study']);
